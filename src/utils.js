...
/**
 * Logs an informational message with the 'Nitro.' prefix.
 *
 ...
 * method name and is automatically prefixed with 'Nitro.'.
 *
 * @param {...any} params - The message components. The first param is used as
 *                          the method name and prefixed with 'Nitro.'.
 */
function log( ...params ) {

	const message = 'Nitro.' + params.shift();
...
function enhanceLogMessage( params ) {
	...
	if ( typeof message === 'string' && message.startsWith( 'TSL:' ) ) {
		...
		if ( stackTrace && stackTrace.isStackTrace ) {
			...
			params[ 1 ] = 'Stack trace not available. Enable "Nitro.Node.captureStackTrace" to capture stack traces.';
...
/**
 * Logs a warning message with the 'Nitro.' prefix.
 *
 * If a custom console function is set via setConsoleFunction(), it will be used
...
/**
 * Logs an error message with the 'Nitro.' prefix.
 *
 * If a custom console function is set via setConsoleFunction(), it will be used
