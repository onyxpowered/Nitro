 ...
 *
 * ```js
 * const camera = new Nitro.PerspectiveCamera( 75, window.innerWidth / window.innerHeight, 0.1, 1000 );
 * const helper = new Nitro.CameraHelper( camera );
 * scene.add( helper );
 * ```
 *
	...
	update() {
		...
		if ( this.camera.reversedDepth === true ) {
			...
			if ( this.camera.coordinateSystem === WebGLCoordinateSystem ) {
				...
				throw new Error( 'Nitro.CameraHelper.update(): Invalid coordinate system: ' + this.camera.coordinateSystem );
