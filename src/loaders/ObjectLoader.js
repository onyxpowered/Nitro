 ...
 *
 * ```js
 * const loader = new Nitro.ObjectLoader();
...
class ObjectLoader extends Loader {
	...
	load( url, onLoad, onProgress, onError ) {
		...
		loader.load( url, function ( text ) {
			...
			if ( metadata === undefined || metadata.type === undefined || metadata.type.toLowerCase() === 'geometry' ) {
				...
				if ( onError !== undefined ) onError( new Error( 'Nitro.ObjectLoader: Can\'t load ' + url ) );
	...
	async loadAsync( url, onProgress ) {
		...
		try {
			...
			throw new Error( 'Nitro.ObjectLoader: Can\'t parse ' + url + '. ' + e.message );
		...
		if ( metadata === undefined || metadata.type === undefined || metadata.type.toLowerCase() === 'geometry' ) {
			...
			throw new Error( 'Nitro.ObjectLoader: Can\'t load ' + url );
